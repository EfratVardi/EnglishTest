(function(window) {
  ...
  var recordWavWorker = new Worker('files/Recordmp3js/js/enc/wav/recorderWorker.js');
  var encoderMp3Worker = new Worker('files/Recordmp3js/js/enc/mp3/mp3Worker.js');
    ...
    */
    ...
    function endFile(blob, extension) {
		if(blob.type == 'audio/mp3'){
			...
			jQuery('#record_element_wrap').append('<audio id="record_element" controls><source src="' + url + '" type="' + blob.type + '" /></audio>');
			sentences_audio_element.play();
			setTimeout(function(){
				var record_element_wrap = document.getElementById('record_element');
				record_element.play();
			}, sentences_audio_element.duration * 1000);
		}
    }
